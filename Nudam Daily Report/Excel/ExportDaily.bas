...
Sub ImportLineDetails()
    ...
    destinWS.Range("A1:J84").Delete
    
    Debug.Print (Now & "    Copying designated range")
    sourceWS.Range("A3:J80").Copy
    ...
    destinWS.Activate
    destinWS.ListObjects.Add(SourceType:=xlSrcRange, Source:=Range("A3:J81"), XlListObjectHasHeaders:=xlNo).Name = "LineDetailsRaw"
...
Sub RefreshQueries()
    ...
    With Excel.Workbooks(dailySummary)
        ...
        With .Worksheets("Daily Summary")
            ...
            .Range("J7:K7").AutoFill Destination:=.Range("J7:K84"), Type:=xlFillValues
            
            ' Shift 2
            .Range("M7").Value = "=[@S2Shot]*[@TotalCavity]"
            .Range("N7").Value = "=[@S2Shot]*[@CycleTime]/28800"
            .Range("M7:N7").AutoFill Destination:=.Range("M7:N84"), Type:=xlFillValues
            
            ' Shift 3
            .Range("P7").Value = "=[@S3Shot]*[@TotalCavity]"
            .Range("Q7").Value = "=[@S3Shot]*[@CycleTime]/28800"
            .Range("P7:Q7").AutoFill Destination:=.Range("P7:Q84"), Type:=xlFillValues
            
            .Range("S7").Value = "=SUM([@S1Qty],[@S2Qty],[@S3Qty])"         ' Total
            .Range("T7").Value = "=AVERAGE([@S1Urt],[@S2Urt],[@S3Urt])"     ' U-Rate
            .Range("S7:T7").AutoFill Destination:=.Range("S7:T84"), Type:=xlFillValues
...
Sub ExportDailyCopy(specificDate As Boolean)
    
    ' Strings for file names and paths
    Dim rootPath As String, _
        targetDate As String, _
    ...
    If specificDate Then
        targetDate = sourceWB.Worksheets("Daily Summary").Range("M1").Value
        targetDate = Format(targetDate, "yyyy-mm-dd")
    Else
        targetDate = Format(Date - 1, "yyyy-mm-dd")
    End If
    
    Debug.Print (Now & "    File Date: " & targetDate)
    
    ' Path\To\File\yyyy Prod Report\yyyy-mm-dd Production Line Status - Daily Printed.xlsx
    folderPath = rootPath & "\" & Format(targetDate, "yyyy") & " Prod Report" & "\"
    ocFolderPath = folderPath & "OriginalCopy" & "\"

    ocFileName = targetDate & ".xlsx"
    fileNameStr = targetDate & " Production Line Status - Daily Printed.xlsx"
...
Sub Run(Optional specificDate As Boolean = False)
